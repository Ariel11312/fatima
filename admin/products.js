document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", function (event) {
    if (event.target.classList.contains("adjust-btn")) {
      ...
      document.querySelector("#current-quantity").value = quantity;
      document.querySelector("#adjust-item-name").value = name;
      document.querySelector("#adjust-item-id").value = itemsku;
      document.querySelector("#adjust-item-category").value = category;
      const adjustType = document.querySelector("#adjustment-type").value;
      adjustQuantityModal.style.display = "block";
      document.body.style.overflow = "hidden";
      // Get all relevant elements
      const adjustTypeSelect = document.querySelector("#adjustment-type");
      const adjustmentItemInput = document.querySelector("#adjustment-item");
      const currentQuantityInput = document.querySelector("#current-quantity");
      const newQuantityInput = document.querySelector("#new-quantity");

      // Function to calculate and update the new quantity
      function updateNewQuantity() {
        const adjustType = adjustTypeSelect.value;
        const quantity = parseInt(currentQuantityInput.value) || 0;
        const adjustment = parseInt(adjustmentItemInput.value) || 0;

        if (adjustType === "add") {
          newQuantityInput.value = quantity + adjustment;
        } else if (adjustType === "subtract") {
          newQuantityInput.value = Math.max(0, quantity - adjustment); // Prevent negative values
        }
      }

      // Add event listeners for both the adjustment type and adjustment value
      adjustTypeSelect.addEventListener("change", updateNewQuantity);
      adjustmentItemInput.addEventListener("input", updateNewQuantity);

      // Initial calculation
      updateNewQuantity();

      document
        .querySelector("#confirmAdjustQuantity")
        .addEventListener("click", function () {
          $.ajax({
            url: "http://localhost/fatima/admin/adjust-quantity.php",
            method: "POST",
            data: {
              item_id: itemsku,
              newQuantity: newQuantityInput.value,
              category: category,
            },
  ...
  document
    .getElementById("edit-item-image")
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      formData.append(
        "subCategory",
        document.getElementById("edit-item-subcategory").value
      );
  ...
  // Confirm delete item
  document
    .getElementById("confirmDeleteItem")
    .addEventListener("click", function () {
      ...
      $.ajax({
        ...
        method: "POST",
        data: { item_id: itemId, category: category },
        dataType: "json",
        success: function (response) {
  ...
  function viewItem(itemId) {
    ...
    if (item) {
      ...
      document.getElementById("view-item-subcategory").textContent =
        item.subCategory;
      ...
      document.getElementById("view-item-status").textContent = formatStatus(
        item.status
      );
      document.getElementById("view-item-status").className =
        "detail-value " + item.status;

      document.getElementById("view-item-lastUpdated").textContent = formatDate(
        item.lastUpdated
      );
      document.getElementById("view-item-description").textContent =
  ...
  function updateSKU() {
    ...
    if (category && subcategory) {
      ...
      const categoryPrefix =
        categoryPrefixMap[prefix] || prefix.substring(0, 3).toUpperCase();
  ...
  function renderInventory(items, page = 1) {
    ...
    document.getElementById("instock").textContent = items.filter(
    ...
    document.getElementById("outstock").textContent = items.filter(
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      row.innerHTML = `
        ...
        item.name
            ...
            <td><span class="status ${item.status}">${formatStatus(
        item.status
      )}</span></td>
            ...
            <button class="adjust-btn" >Adjust Qty</button>
            </div>
            </td>`;
